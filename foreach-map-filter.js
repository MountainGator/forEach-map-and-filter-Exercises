...
function showFirstAndLast(arr){
    const firstAndLast = arr.map(function(w){
...
function vowelCount(str){
   ...
   const isVowel = function (char) {
    ...
    const justVowels = words.filter(isVowel);
    const counts = {};
    justVowels.forEach(function (v){
...
function valTimesIndex(arr){
    const multi = arr.map(function (p, i) {
        return p * i;
    })
...
function extractKey(arr, key){
    const extraction = arr.map(function (n){
        return n.name;
    })
...
function extractFullName(arr){
    ...
    const getNames = arr.map(function (n){
